...
export const getToken = async token => (await db.ref(`/tokens/${token}`).once('value')).val()

export const isValidToken = async token => (await getToken(token)) !== null

export const isTokenFreezed = async token => !((await getToken(token) !== 0) && (await isValidToken(token)))
